...
export class DashboardPage {
  ...
  processWorkoutChart() {
    ...
    const workoutsSorted = [...this.workouts].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  
    workoutsSorted.forEach(workout => {
      console.log('Workout:', workout);
      const total = workout.exercises.reduce((sum: number, ex: any) =>
        sum + (ex.reps * ex.sets * ex.weight), 0
      );
  ...
  processBodyChart() {
    ...
    this.bodyChartLabels = sorted.map(m => new Date(m.date).toLocaleDateString());
    this.bodyChartData = {
